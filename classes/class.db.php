...
public function get_select($id,$table,$row='name',$where='actived = 1'){
    ...
    $data .= '';
    foreach ( $this ->getResults($queryString) as $result ) {
        if ($result['id'] == $id) {
            $data .= '<option value="' . $result['id'] . '" selected="selected">' . $result[$row] . '</option>';
        } else {
            $data .= '<option value="' . $result['id'] . '">' . $result[$row] . '</option>';
        }
    }
